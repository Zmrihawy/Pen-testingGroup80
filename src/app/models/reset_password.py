...
def search_for_user(token, ip, fullpath):
    """
    ...
    try:
        ...
        logger.log_error_msg("Failed executing query search_for_user: {}".format(err))
        print("Failed executing query search_for_user: {}".format(err))
        cursor.fetchall()
        exit(1)
    finally:
        cursor.close()
